...
import { emitParticle, spriteInstancesTick } from "../sprites/sprites";
...
export function tick(world: WorldState, events: Event[]) {
  ...
  world.bombs = world.bombs.reduce<Bomb[]>((result, b) => {
    ...
    if (b.countdown < 0) {
      ...
      emitParticle(SpriteEnum.FIREBALL, b.tx, b.ty);
    }
    return [...result, ...(b.countdown < 0 ? [] : [b])];
  }, []);
  ...
  if (world.time % 10 == 0) {
    ...
    world.blasts.forEach((b) => {
      ...
      around.forEach((a) => {
        ...
        if (!b.expanded && ((b.powL > 0 && a.ox < 0) || (b.powR > 0 && a.ox > 0) || (b.powU > 0 && a.oy < 0) || (b.powD > 0 && a.oy > 0))) {
          // hits a player?
          ...
          if (adjacent.type === TileTypes.GROUND) {
            ...
            emitParticle(SpriteEnum.FIREBALL, tx, ty);
          }
        }
      });
      b.expanded = true;
  ...
  // animations ticks
  spriteInstancesTick(world.time);
}
